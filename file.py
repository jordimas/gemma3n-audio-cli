...
def transcribe_chunk(processor, model, prompt, chunk, temperature):
    messages = [
        {
            "role": "user",
            "content": [
    ...
    end_time = time.time()
    print(f"transcribe_chunk: {end_time - start_time:.2f} seconds")
...
def process_file(filename, model, processor, prompt, temperature=0.1, use_vad=False):
    ...
    for i, chunk in enumerate(chunks):
        ...
        answer = transcribe_chunk(processor, model, prompt, temp_filename, temperature)
